# 💻 Notion 페이지 작성 코드 패턴 (범용)

> 💡 **범용 가이드**: 모든 MCP 지원 AI 플랫폼에서 사용 가능

## 📋 TL;DR (핵심 요약)

**목적**: 실행 가이드에서 사용할 코드 패턴 및 함수 제공

**주요 함수**:

1. `formatPageTitle()` - 제목 포맷팅
2. `splitIntoChunks()` - 청크 분할
3. `copyToClipboard()` - 클립보드 복사
4. `scrollAndFocus()` - 스크롤 및 포커스
5. `verifyContent()` - 내용 검증
6. `parseParentPageId()` - 부모 페이지 URL/ID 파싱
7. `resolveFilePath()` - 파일 경로 해석

---

## 🎯 제목 포맷팅 함수

**Input**: 파일명 (string)  
**Output**: 포맷팅된 제목 (string)

**규칙**:

1. 파일명에서 `.md` 확장자 제거
2. `_` → 공백 변환
3. 연속 공백 → 단일 공백
4. 앞뒤 공백 제거

### JavaScript 버전

```javascript
// 파일명에서 확장자 제거 및 제목 포맷팅
function formatPageTitle(fileName) {
  // 1. 확장자 제거
  let pageTitle = fileName.replace(/\.md$/, "");

  // 2. 언더스코어를 공백으로 변환
  pageTitle = pageTitle.replace(/_/g, " ");

  // 3. 연속된 공백 제거 및 앞뒤 공백 제거
  pageTitle = pageTitle.replace(/\s+/g, " ").trim();

  return pageTitle;
}

// 사용 예시
const fileName = "노션_셋업_가이드.md";
const pageTitle = formatPageTitle(fileName);
// 결과: "노션 셋업 가이드"
```

### Python 버전

```python
import re

def format_page_title(file_name):
    # 1. 확장자 제거
    page_title = file_name.replace('.md', '') if file_name.endswith('.md') else file_name

    # 2. 언더스코어를 공백으로 변환
    page_title = page_title.replace('_', ' ')

    # 3. 연속된 공백 제거 및 앞뒤 공백 제거
    page_title = re.sub(r'\s+', ' ', page_title).strip()

    return page_title

# 사용 예시
file_name = "노션_셋업_가이드.md"
page_title = format_page_title(file_name)
# 결과: "노션 셋업 가이드"
```

---

## ✂️ 청크 분할 함수

**Input**: 파일 내용 (string), 라인 수 (number)  
**Output**: 청크 배열 (string[])

**규칙**:

- < 500줄: 1개 청크
- 500-2,000줄: 200-300줄 단위
- > 2,000줄: 300-500줄 단위
- 각 청크는 최대 2,000자

### JavaScript 버전

```javascript
function splitIntoChunks(content, lineCount) {
  const lines = content.split("\n");
  const chunks = [];

  let chunkSize;
  if (lineCount < 500) {
    chunkSize = lineCount; // 1개 청크
  } else if (lineCount <= 2000) {
    chunkSize = 250; // 200-300줄 단위
  } else {
    chunkSize = 400; // 300-500줄 단위
  }

  for (let i = 0; i < lines.length; i += chunkSize) {
    const chunk = lines.slice(i, i + chunkSize).join("\n");

    // 2,000자 제한 확인
    if (chunk.length > 2000) {
      // 더 작은 청크로 분할
      const subChunks = chunk.match(/.{1,2000}/g) || [];
      chunks.push(...subChunks);
    } else {
      chunks.push(chunk);
    }
  }

  return chunks;
}
```

---

## 📋 클립보드 복사 함수

**Input**: 텍스트 (string, 최대 2,000자)  
**Output**: 성공 여부 (boolean), 에러 메시지 (선택적, string)

**브라우저 도구**: `browser_evaluate`

### JavaScript 패턴 (browser_evaluate에서 사용)

```javascript
// browser_evaluate에서 사용할 패턴
async () => {
  const text = `[청크 내용]`; // 최대 2,000자

  try {
    await navigator.clipboard.writeText(text);
    return {
      success: true,
      message: "클립보드에 복사 완료",
      chunkSize: text.length,
    };
  } catch (err) {
    return {
      success: false,
      error: err.message,
    };
  }
};
```

**재시도 로직** (최대 3회):

```javascript
async function copyToClipboardWithRetry(text, maxRetries = 3) {
  for (let i = 0; i < maxRetries; i++) {
    try {
      await navigator.clipboard.writeText(text);
      return { success: true };
    } catch (err) {
      if (i === maxRetries - 1) {
        return { success: false, error: err.message };
      }
      await new Promise((resolve) => setTimeout(resolve, 1000));
    }
  }
}
```

---

## 📜 페이지 하단 스크롤 및 포커스

**브라우저 도구**: `browser_evaluate`

### JavaScript 패턴

```javascript
// browser_evaluate에서 사용할 패턴
() => {
  // 1. 페이지 하단으로 스크롤
  window.scrollTo(0, document.body.scrollHeight);

  // 2. 편집 영역 찾기 및 포커스
  const textbox = document.querySelector('textbox[aria-label*="입력"]');
  if (textbox) {
    textbox.focus();
    textbox.click();
    return { success: true };
  }

  return { success: false, error: "편집 영역을 찾을 수 없습니다" };
};
```

---

## 🔍 내용 검증 함수

**Input**: 파일의 마지막 줄 텍스트 (string)  
**Output**: 검증 결과 (boolean)

**브라우저 도구**: `browser_evaluate`

### JavaScript 패턴

```javascript
// browser_evaluate에서 사용할 패턴
(lastLine) => {
  const pageText = document.body.innerText;
  const hasLastLine = pageText.includes(lastLine);

  return {
    success: hasLastLine,
    pageLength: pageText.length,
    hasLastLine: hasLastLine,
  };
};
```

---

## ✅ 제목 입력 검증 함수

**목적**: 제목이 정확히 입력되었는지 확인

**Input**: 예상 제목 (string)  
**Output**: 검증 결과 (object)

**브라우저 도구**: `browser_evaluate`

### JavaScript 패턴

```javascript
// 제목 입력 검증
function verifyTitleInput(expectedTitle) {
  return browser_evaluate((expected) => {
    const titleElement = document.querySelector('h1[contenteditable="true"]');
    const actualTitle = titleElement?.innerText.trim();
    
    // 제목 일치 여부 확인
    if (actualTitle !== expected) {
      return {
        success: false,
        error: "ERR_TITLE_MISMATCH",
        expected: expected,
        actual: actualTitle,
        message: "제목이 정확히 입력되지 않았습니다"
      };
    }
    
    // 명령어 포함 여부 확인
    if (actualTitle.includes('/page') || actualTitle.includes('Ctrl+')) {
      return {
        success: false,
        error: "ERR_TITLE_HAS_COMMAND",
        actual: actualTitle,
        message: "제목에 명령어가 포함되어 있습니다"
      };
    }
    
    return { 
      success: true, 
      title: actualTitle,
      message: "제목 입력 검증 완료"
    };
  }, expectedTitle);
}
```

---

## 🎯 내용 영역 이동 확인 함수

**목적**: 제목 영역에서 완전히 벗어나 내용 영역으로 이동

**Output**: 이동 결과 (object)

**브라우저 도구**: `browser_evaluate`

### JavaScript 패턴

```javascript
// 내용 영역으로 안전하게 이동
function moveToContentArea() {
  return browser_evaluate(() => {
    // 1. 제목 영역 확인
    const titleElement = document.querySelector('h1[contenteditable="true"]');
    const isFocusedOnTitle = titleElement?.contains(document.activeElement);
    
    if (isFocusedOnTitle) {
      // 제목 영역에서 벗어나기
      const bodyElement = document.querySelector('[contenteditable="true"]:not(h1)');
      if (bodyElement) {
        bodyElement.focus();
        bodyElement.click();
      } else {
        return {
          success: false,
          error: "내용 영역을 찾을 수 없습니다"
        };
      }
    }
    
    // 2. 내용 영역 확인
    const contentArea = document.querySelector('[contenteditable="true"]:not(h1)');
    if (!contentArea) {
      return {
        success: false,
        error: "내용 영역을 찾을 수 없습니다"
      };
    }
    
    // 3. 최하단으로 이동
    contentArea.focus();
    const range = document.createRange();
    range.selectNodeContents(contentArea);
    range.collapse(false); // 끝으로
    
    const selection = window.getSelection();
    selection.removeAllRanges();
    selection.addRange(range);
    
    return {
      success: true,
      isTitleArea: false,
      message: "내용 영역으로 이동 완료"
    };
  });
}
```

---

## 🔒 청크 입력 전 검증 함수 (제목 보호)

**목적**: 청크 입력 전 제목 영역 보호 및 위치 확인

**Input**: 예상 제목 (string)  
**Output**: 검증 결과 (object)

**브라우저 도구**: `browser_evaluate`

### JavaScript 패턴

```javascript
// 청크 입력 전 검증
function preChunkUploadCheck(expectedTitle) {
  return browser_evaluate((expected) => {
    const titleElement = document.querySelector('h1[contenteditable="true"]');
    const currentTitle = titleElement?.innerText.trim();
    const isFocusedOnTitle = titleElement?.contains(document.activeElement);
    
    // 1. 제목 영역에 포커스되어 있는지 확인
    if (isFocusedOnTitle) {
      return {
        success: false,
        error: "ERR_FOCUSED_ON_TITLE",
        message: "제목 영역에 포커스됨 - 내용 영역으로 이동 필요"
      };
    }
    
    // 2. 제목이 변경되지 않았는지 확인
    if (currentTitle !== expected) {
      return {
        success: false,
        error: "ERR_TITLE_CHANGED",
        expected: expected,
        actual: currentTitle,
        message: "제목이 변경됨 - 중단 필요"
      };
    }
    
    return { 
      success: true,
      message: "입력 전 검증 완료"
    };
  }, expectedTitle);
}
```

---

## 🔍 청크 입력 후 검증 함수 (제목 + 연결성)

**목적**: 청크 입력 후 제목 보호 및 이전 청크와 연결성 확인

**Input**: 
- 예상 제목 (string)
- 이전 청크 마지막 부분 (string, 선택적)
- 현재 청크 시작 부분 (string, 선택적)

**Output**: 검증 결과 (object)

**브라우저 도구**: `browser_evaluate`

### JavaScript 패턴

#### 1. 제목 변경 확인

```javascript
// 청크 입력 후: 제목 확인
function postChunkUploadTitleCheck(expectedTitle) {
  return browser_evaluate((expected) => {
    const titleElement = document.querySelector('h1[contenteditable="true"]');
    const currentTitle = titleElement?.innerText.trim();
    
    if (currentTitle !== expected) {
      return {
        success: false,
        error: "ERR_TITLE_CORRUPTED",
        expected: expected,
        actual: currentTitle,
        message: "제목이 변경됨 - 복구 필요"
      };
    }
    
    return { 
      success: true, 
      title: currentTitle,
      message: "제목 보호 확인 완료"
    };
  }, expectedTitle);
}
```

#### 2. 청크 연결성 확인 (2번째 청크부터)

```javascript
// 청크 입력 후: 이전 청크와 연결성 확인
function verifyChunkContinuity(previousChunkEnd, currentChunkStart) {
  return browser_evaluate((prevEnd, currStart) => {
    const bodyText = document.body.innerText;
    
    // 1. 이전 청크의 마지막 부분이 있는지 확인
    const hasPreviousEnd = bodyText.includes(prevEnd);
    
    // 2. 현재 청크의 시작 부분이 있는지 확인
    const hasCurrentStart = bodyText.includes(currStart);
    
    // 3. 순서가 올바른지 확인 (이전 끝 → 현재 시작)
    const previousEndIndex = bodyText.indexOf(prevEnd);
    const currentStartIndex = bodyText.indexOf(currStart);
    const isCorrectOrder = previousEndIndex !== -1 && 
                           currentStartIndex !== -1 && 
                           previousEndIndex < currentStartIndex;
    
    if (!hasPreviousEnd || !hasCurrentStart) {
      return {
        success: false,
        error: "ERR_CHUNK_MISSING",
        hasPreviousEnd: hasPreviousEnd,
        hasCurrentStart: hasCurrentStart,
        message: "청크가 누락됨"
      };
    }
    
    if (!isCorrectOrder) {
      return {
        success: false,
        error: "ERR_CHUNK_ORDER",
        previousEndIndex: previousEndIndex,
        currentStartIndex: currentStartIndex,
        message: "청크 순서가 잘못됨"
      };
    }
    
    return { 
      success: true,
      message: "청크 연결성 확인 완료",
      previousEndIndex: previousEndIndex,
      currentStartIndex: currentStartIndex,
      distance: currentStartIndex - previousEndIndex
    };
  }, previousChunkEnd, currentChunkStart);
}
```

#### 3. 통합 검증 함수

```javascript
// 청크 업로드 통합 검증 (제목 + 연결성)
async function postChunkUploadVerification(
  expectedTitle, 
  previousChunkEnd = null, 
  currentChunkStart = null
) {
  // 1. 제목 확인
  const titleCheck = await postChunkUploadTitleCheck(expectedTitle);
  if (!titleCheck.success) {
    return titleCheck;
  }
  
  // 2. 연결성 확인 (2번째 청크부터)
  if (previousChunkEnd && currentChunkStart) {
    const continuityCheck = await verifyChunkContinuity(
      previousChunkEnd, 
      currentChunkStart
    );
    if (!continuityCheck.success) {
      return continuityCheck;
    }
    
    return {
      success: true,
      titleCheck: titleCheck,
      continuityCheck: continuityCheck,
      message: "전체 검증 완료"
    };
  }
  
  // 첫 번째 청크는 제목만 확인
  return titleCheck;
}
```

### 사용 예시

```javascript
// 청크 업로드 예시
const chunks = ["청크1", "청크2", "청크3"];
const expectedTitle = "문서 제목";

for (let i = 0; i < chunks.length; i++) {
  // 입력 전 검증
  const preCheck = await preChunkUploadCheck(expectedTitle);
  if (!preCheck.success) {
    throw new Error(preCheck.error);
  }
  
  // 청크 입력
  await uploadChunk(chunks[i]);
  
  // 입력 후 검증
  let postCheck;
  if (i === 0) {
    // 첫 번째 청크: 제목만 확인
    postCheck = await postChunkUploadTitleCheck(expectedTitle);
  } else {
    // 2번째 이후: 제목 + 연결성 확인
    const prevEnd = chunks[i - 1].slice(-50);
    const currStart = chunks[i].slice(0, 50);
    postCheck = await postChunkUploadVerification(
      expectedTitle,
      prevEnd,
      currStart
    );
  }
  
  if (!postCheck.success) {
    throw new Error(postCheck.error);
  }
  
  console.log(`청크 ${i + 1}/${chunks.length} 업로드 및 검증 완료`);
}
```

---

## 🔗 부모 페이지 URL 파싱 함수

**Input**: URL 또는 UUID (string)  
**Output**: 페이지 ID (string, 하이픈 없음)

**지원 형식**:

- `https://www.notion.so/2939168184838094b94bc4ad6aab8c88`
- `https://purple-paint-fa5.notion.site/...`
- `29391681-8483-8094-b94b-c4ad6aab8c88`

### JavaScript 버전

```javascript
function parseParentPageId(input) {
  // 1. URL 형식인지 확인
  if (input.startsWith("http")) {
    try {
      const url = new URL(input);
      const pathParts = url.pathname.split("/").filter((p) => p);

      // 마지막 경로 부분이 페이지 ID
      let pageId = pathParts[pathParts.length - 1];

      // 앵커 제거
      pageId = pageId.split("#")[0];

      // 하이픈 제거
      pageId = pageId.replace(/-/g, "");

      return pageId;
    } catch (e) {
      throw new Error(`URL 파싱 실패: ${input}`);
    }
  }

  // 2. UUID 형식인 경우 (하이픈 제거)
  return input.replace(/-/g, "");
}
```

---

## 📁 파일 경로 해석 함수

**Input**: 파일 경로 (string)  
**Output**: 해석된 파일 경로 (string)

**처리 우선순위**:

1. 절대 경로 (그대로 사용)
2. 상대 경로 (워크스페이스 루트 기준)
3. 파일명만 (워크스페이스 전체 검색 필요)

### JavaScript 버전

```javascript
function resolveFilePath(inputPath, workspaceRoot) {
  // 1. 절대 경로인지 확인
  if (inputPath.startsWith("/") || inputPath.match(/^[A-Za-z]:/)) {
    return inputPath; // Windows: C:\..., Unix: /...
  }

  // 2. 상대 경로 처리
  if (inputPath.startsWith("./") || inputPath.includes("/")) {
    // 워크스페이스 루트 기준으로 해석
    return workspaceRoot + "/" + inputPath.replace(/^\.\//, "");
  }

  // 3. 파일명만 (검색 필요)
  return inputPath; // glob_file_search로 검색 필요
}
```

---

## 🛡️ 기존 내용 확인 함수

**목적**: 기존 페이지의 내용을 보존하기 위한 안전장치

**브라우저 도구**: `browser_evaluate`

### 제목 영역 기존 내용 확인

```javascript
// browser_evaluate에서 사용
() => {
  const titleElement =
    document.querySelector('h1[contenteditable="true"]') ||
    document.querySelector('[contenteditable="true"]');

  if (!titleElement) {
    return { success: false, error: "제목 영역을 찾을 수 없습니다" };
  }

  const existingTitle = titleElement.innerText.trim();

  // 기존 제목이 있고 "새 페이지" 또는 "Untitled"가 아니면 보존
  if (
    existingTitle &&
    existingTitle !== "새 페이지" &&
    existingTitle !== "Untitled" &&
    existingTitle !== ""
  ) {
    return {
      success: true,
      preserve: true,
      existingTitle: existingTitle,
      message: `기존 제목이 있습니다: "${existingTitle}". 기존 제목을 유지합니다.`,
    };
  }

  return {
    success: true,
    preserve: false,
    message: "새 페이지입니다. 제목 입력을 진행합니다.",
  };
};
```

### 본문 영역 기존 내용 확인

```javascript
// browser_evaluate에서 사용
() => {
  const bodyElements = document.querySelectorAll('[contenteditable="true"]');
  let existingContent = "";
  let contentBlocks = 0;

  // 제목이 아닌 본문 영역 찾기
  for (let elem of bodyElements) {
    if (elem.tagName !== "H1" && elem.innerText.trim() !== "") {
      existingContent += elem.innerText.trim() + " ";
      contentBlocks++;
    }
  }

  if (existingContent.trim().length > 0) {
    return {
      success: true,
      hasExistingContent: true,
      contentLength: existingContent.trim().length,
      contentBlocks: contentBlocks,
      message:
        "기존 내용이 있습니다. 기존 내용을 보존하고 하단에만 추가합니다.",
    };
  }

  return {
    success: true,
    hasExistingContent: false,
    message: "새 페이지입니다. 내용 업로드를 진행합니다.",
  };
};
```

---

## 📚 참조

- **메인 가이드**: `00_메인_가이드.md`
- **실행 가이드**: `01_실행_가이드.md`
- **에러 처리**: `03_에러_처리.md`
- **템플릿**: `04_템플릿.md`

---

**Made with ❤️ for AI Agents (Universal)**

---

## 📝 출처

**작성자**: 김준모  
**이메일**: rnsdlsdmtlk@gmail.com  
**버전**: 2.0.0 (Universal)
